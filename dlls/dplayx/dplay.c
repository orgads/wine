    ...
    case DPMSGCMD_REQUESTPLAYERID:
    {
      ...
      *lplpReply = HeapAlloc( GetProcessHeap(), HEAP_ZERO_MEMORY, *lpdwMsgSize );
      ...
      if ( lpcMsg->Flags & DPLAYI_PLAYER_SYSPLAYER )
      {
        ...
        lpReply->Result = DP_IF_CreatePlayer( This, lpMessageHeader,
...
HRESULT DP_CreatePlayer( IDirectPlayImpl* This, DPID idPlayer, DPNAME *lpName,
        DWORD dwFlags, VOID *lpData, DWORD dwDataSize, HANDLE hEvent, BOOL bAnsi,
        ...
        BOOL bAnsi )
{
  ...
  if ( !This->dp2->bHostInterface && !lpMsgHdr )
  {
     if ( dwFlags & DPLAYI_PLAYER_SYSPLAYER )
     {
        ...
        GUID *instance, LPDPENUMPLAYERSCALLBACK2 enumplayercb, void *context, DWORD flags )
{
    ...
    if ( !enumplayercb || ( ( flags & DPENUMGROUPS_SESSION ) && !instance ) )
    {
        return DPERR_INVALIDPARAMS;
    }
        ...
        DWORD timeout, LPDPENUMSESSIONSCALLBACK2 enumsessioncb, void *context, DWORD flags )
{
    ...
    if( !sdesc || (sdesc->dwSize != sizeof(DPSESSIONDESC2)) )
        return DPERR_INVALIDPARAMS;
